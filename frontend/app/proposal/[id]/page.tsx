...
import { useProposal, useDao, useEvaluateProposal } from "@/lib/hooks/useTreasuryPilot";
...
export default function ProposalPage() {
  ...
  const { data: dao } = useDao(proposal?.dao_id ?? null);
      ...
      <main className="grow pt-14 pb-16">
        ...
        <div className="border-b border-slate-800/80 bg-black/30">
          <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8 py-8">
            <button
              onClick={() => dao ? router.push(`/dao/${dao.id}`) : router.back()}
            ...
            <div className="flex flex-wrap gap-4 mt-3 text-xs font-mono text-slate-600">
              <span>{proposal.target_council}</span>
              <span>·</span>
              <span>{proposal.requested_amount}</span>
              <span>·</span>
              <span>by {proposal.recipient?.slice(0, 10)}...</span>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8 py-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              {[
                { label: "Target Council", value: proposal.target_council },
                { label: "Requested Amount", value: proposal.requested_amount, mono: true },
                { label: "Recipient", value: proposal.recipient, mono: true, truncate: true },
                { label: "Submitter", value: proposal.recipient, mono: true, truncate: true },
              ].map((item) => (
