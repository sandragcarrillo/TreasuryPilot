...
export async function POST(req: Request) {
  return handleRelay<Data, { genlayerTxHash: string }>({
    ...
    validate: (data) => {
      ...
      const appealText = requireString(obj.value.appealText, "appealText", { max: 8192 });
      if (!appealText.ok) return appealText;
    ...
    execute: async ({ actor, data }) => {
      const tx = await genlayerRelay.fileAppeal(actor, data);
